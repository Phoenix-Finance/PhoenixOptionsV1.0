 ...
 *
 */
contract phxAutoMinePool is MinePoolData {
    ...
    function setStakeCoin(address _stakeCoin) public originOnce{
        ...
        mineNetworth[_stakeCoin] = rayDecimals;
    }
    /**
     ...
     * @param mineCoin mineCoin address
     */
    function getTotalMined(address mineCoin)public notStakeCoin(mineCoin) view returns(uint256){
     ...
     * @param mineCoin mineCoin address
     */
    function getMinerBalance(address account,address mineCoin)public notStakeCoin(mineCoin) view returns(uint256){
    ...
    function stake(uint256 amount)public nonReentrant notHalted {
        ...
        amount = amount.mul(rayDecimals)/getNetWorth(stakeCoin);
    ...
    function unstakeAll()public nonReentrant notHalted {
        _unstake(getStakeBalance(msg.sender));
    }
    ...
    function setStakeInterest(uint256 _mineAmount,uint256 _mineInterval)public onlyOrigin {
        require(_mineAmount<1e30,"input mine amount is too large");
        require(_mineInterval>0,"input mine Interval must larger than zero");
        _globalSettlement(stakeCoin);
        mineAmount[stakeCoin] = _mineAmount;
        mineInterval[stakeCoin] = _mineInterval;
        emit SetMineCoinInfo(msg.sender,stakeCoin,_mineAmount,_mineInterval);
    }
    /**
     ...
     */
    function setMineCoinInfo(address mineCoin,uint256 _mineAmount,uint256 _mineInterval)public notStakeCoin(mineCoin) onlyOrigin {
        require(_mineAmount<1e30,"input mine amount is too large");
        require(_mineInterval>0,"input mine Interval must larger than zero");
     ...
     */
    function redeemMinerCoin(address mineCoin)public notStakeCoin(mineCoin) nonReentrant notHalted {
        _globalSettlement(mineCoin);
